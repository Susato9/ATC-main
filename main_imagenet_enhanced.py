...
import time
import numpy as np
from clip.model import ResTask_enhanced
import torch

class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=20):
        super(ChannelAttention, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool1d(1024)
        self.max_pool = nn.AdaptiveMaxPool1d(1024)
...
class ConNet(nn.Module):
    def __init__(self, in_planes, ratio=20,embed_dim=1024):
        super(ConNet, self).__init__()
        self.ChannelAttention=ChannelAttention(in_planes, ratio)
        self.lstm=nn.LSTM(embed_dim,embed_dim,3,batch_first=True)
    def forward(self, x):
        x=x.unsqueeze(1).repeat(1,1000,1)
...
def ATC(cfg, cache_keys, cache_values, test_features, test_labels, clip_weights, clip_model, train_loader_F,test_loader,imagenet,params):
    ...
    # guide_net=ConNet(in_planes=1000, ratio=20,embed_dim=1024).to(clip_model.dtype).cuda()
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            with torch.no_grad():
                image_features= clip_model.encode_image(images)  #restask_txt:(batch_size,512)
...
def main():
    ...
    pretrain_enhance=" "
    model=ResTask_enhanced(cfg,clip_model,pretrain_enhance)
   
    for name, param in model.named_parameters():
        if "image_mask" not in name and "image_query" not in name:
