...
import clip
from utils import *
from clip.model import linearCLIP,ResTask,ResTask_enhanced


def get_arguments():

    parser = argparse.ArgumentParser()
    parser.add_argument('--config', dest='config', default="configs/caltech101.yaml" ,help='settings of Tip-Adapter in yaml format')
    args = parser.parse_args()
    return args



def ATC(cfg, cache_keys, cache_values, test_features, test_labels, clip_weights, model, train_loader_F,test_loader,params):
    ...
    if cfg['backbone']=='RN50':
        ...
        embed_dim=512
    guide_net=nn.LSTM(embed_dim,embed_dim,3,batch_first=True).to(model.dtype).cuda()
    

    optimizer1 = torch.optim.AdamW(params, lr=0.001,eps=1e-3)
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            with torch.no_grad():
                image_features= model.encode_image(images)  #restask_txt:(batch_size,512)
            ...
            mask_txt=txt_emb.repeat(cfg["Number_of_categories"],1).permute(1,0)
            
    
            clip_logits = 100. * (image_features)@ (clip_weights+mask_txt)
            
            affinity=image_features@(cache_keys+model.image_mask.half().cuda())
        ...
        print("adapter",model.image_mask)
        acc_all=correct_samples*100./all_samples
        if acc_all > best_acc:
            best_acc = acc_all
            best_epoch = train_idx     
            torch.save(model.state_dict(), cfg['workdir'] + "/best_F_" + str(cfg['backbone']).replace("/","_")+str(cfg['shots']) + "shots_clip_model.pt")
            torch.save(guide_net.state_dict(), cfg['workdir'] + "/best_F_" +cfg['backbone'].replace("/","_")+ str(cfg['shots']) + "shots_guide_net.pt")
            torch.save(model.image_mask, cfg['workdir'] + "/best_F_" +cfg['backbone'].replace("/","_")+ str(cfg['shots']) + "shots_image_mask.pt")
            best_mask=mask_txt
            best_image_mask=model.image_mask
        
        print("**** ATC's test accuracy: {:.4f}. ****\n".format(acc_all))
        with open(cfg['workdir']+"/"+log_name,"a+") as f:
            f.write(str(acc_all)+"\n")
            
    model.load_state_dict(torch.load(cfg['workdir'] + "/best_F_" +cfg['backbone'].replace("/","_")+ str(cfg['shots']) + "shots_clip_model.pt"))#Load the best clip_model model.
...
def main():
    ...
    for name, param in model.named_parameters():
        if "image_mask" not in name:
            ...
            print(name)
    params = filter(lambda p: p.requires_grad, model.parameters()) # Return all parameters with requires_grad=True.
    
    for name, value in model.named_parameters():
        print(name, value.requires_grad)  # Print the requires_grad attribute of all parameters, True or False.
    # Prepare dataset
    random.seed(1)
    torch.manual_seed(1)
    ...
    # ------------------------------------------ ATC ------------------------------------------
    ATC(cfg, cache_keys, cache_values, test_features, test_labels, clip_weights, model, train_loader_F,test_loader,params)
